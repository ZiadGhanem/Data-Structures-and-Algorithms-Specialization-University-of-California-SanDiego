...
int main(void){
  int u,v,c,n,m,t;
  assert(scanf("%d %d", &n, &m) == 2);
  DistPreprocessSmall dist_preprocess_small(n);

  for (int i = 0; i < m; ++i) {
      assert(scanf("%d %d %d", &u, &v, &c) == 3);
      dist_preprocess_small.add_edge(u-1, v-1, c);
  }

  dist_preprocess_small.preprocess();
  cout << "Ready" << endl;
  ...
  for (int i = 0; i < t; ++i) {
      int u, v;
      assert(scanf("%d %d", &u, &v) == 2);
      printf("%d\n", dist_preprocess_small.query(u - 1, v - 1, i));
  }
}
